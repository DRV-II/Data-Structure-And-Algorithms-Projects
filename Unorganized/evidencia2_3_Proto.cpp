...
//Fecha de creación: 11 de septiembre del 2021
...
void pruebaError(vector<vector<string>> a, int i)
{
  ...
  if (i < a.size())
  {
    ...
    throw invalid_argument("Sobrepasa Indice");
  }
}
...
int busqSecuencialOrd(vector<vector<string>> a, int k)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    try
    {
      vector<string> execpcion = pruebaError(a, i);
    }
    catch(const error& e)
    {
      interruptor = 1;
    ...
    */
    ...
    if ((k <= (stoi(a[i][0]) + stoi(a[i][1]))) || (((k == (stoi(a[i][0]) + stoi(a[i][1]))+1) && ((k==131)||(k==228)||(k==331)||(k==430)||(k==531)||(k==630)||(k==730)||(k==831)||(k==930)||(k==1031)||(k==1130)||(k==1231)))&&((interruptor == 1) || (k != ((stoi(a[i+1][0]) + stoi(a[i+1][1]))+1)))))
    {
      ...
      {
        if (k == (stoi(a[i][0]) + stoi(a[i][1]))+1) { // Porque bitacora a veces no tiene los ultimos días del mes
          return i;
        }
        else {
          return -1;
...
// Formato a la ip para facilitar su lectura.
...
int main()
{
  ...
  cout<<"Compilando... Tiempo de espera aproximado de 6 min"<<endl;
  leerArchivo("bitacora.txt", bitacora);

  vectorSeparado = separar(bitacora); // ip se encuentra en el indice 3
  cout<<vectorSeparado[0][3]<<endl; // ip de primera linea

  vectorConvertido = convertirNumeros(vectorSeparado);

  // Convertir ip a enteros
  vectorConvertido = convertirIPS(vectorConvertido, posiciones);
  cout<<vectorConvertido[0][3]<<endl;
  cout<<posiciones[0][0]<<endl;

  vectorConvertido = convertirIPS2(vectorConvertido);

  cout<<vectorConvertido[0][3]<<endl;
  cout<<vectorConvertido[1][3]<<endl;
  cout<<vectorConvertido[2][3]<<endl;

  vectorOrdenado = ordenBurbuja(vectorConvertido, posiciones);

  cout << endl<<"Ingresa el mes de inicio (numero de mes): "; //Se ingresa el primer valor a buscar.
  cin >> mes;
  ...
  if ((indice2==131)||(indice2==228)||(indice2==331)||(indice2==430)||(indice2==531)||(indice2==630)||(indice2==730)||(indice2==831)||(indice2==930)||(indice2==1031)||(indice2==1130)||(indice2==1231))
  {
